...
import { Search, Plus, Grid, LayoutList, Filter } from "lucide-react"
            ...
            <Tabs defaultValue="all" className="w-full">
              <TabsList className="grid w-full grid-cols-4 mb-6">
                ...
                <TabsTrigger value="color-profiles">
                  Color Profile ({mockDataItems.filter((item) => item.type_id === "1").length})
                </TabsTrigger>
                <TabsTrigger value="presets">
                  Preset ({mockDataItems.filter((item) => item.type_id === "2").length})
                </TabsTrigger>
                <TabsTrigger value="color-styles">
                  Color Style ({mockDataItems.filter((item) => item.type_id === "3").length})
                </TabsTrigger>
              </TabsList>
                        ...
                        <CardContent className="p-4">
                          ...
                          <div className="flex items-center justify-between text-sm">
                            <Badge variant="secondary">
                              {mockDataTypes.find((type) => type.id === item.type_id)?.name}
                            </Badge>
              ...
              <TabsContent value="presets" className="space-y-4">
                <EmptyState title="No presets found" description="Create your first preset to get started." />
              </TabsContent>

              <TabsContent value="color-styles" className="space-y-4">
                <EmptyState title="No color styles found" description="Create your first color style to get started." />
              </TabsContent>
            </Tabs>
          </div>
          {selectedDataType && (
