...
export default function ColorStylesPage() {
  ...
  const dataType = mockDataTypes.find((type) => type.slug === "color-styles")!
  const styleItems = mockDataItems.filter((item) => item.type_id === dataType.id)

  const filteredItems = styleItems.filter(
    (item) =>
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1">
          <div className="container py-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-3xl font-bold">Color Styles</h1>
                <p className="text-muted-foreground">Color grading styles and looks</p>
              </div>
            ...
            {filteredItems.length > 0 ? (
                  ...
                  <Card key={item.id} className="group hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center justify-between text-sm mt-3 pt-3 border-t">
                        <Badge variant="secondary">Color Style</Badge>
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">No color styles found</h3>
                <p className="text-muted-foreground max-w-sm mb-4">
                  {searchQuery
                    ? "No color styles match your search criteria."
                    : "Create your first color style to get started."}
                </p>
                <Button onClick={() => setIsItemFormOpen(true)}>
                  <Plus className="h-4 w-4 mr-2" />
      ...
      <DataItemForm
        ...
        onSave={(item) => {
          console.log("Saving color style:", item)
